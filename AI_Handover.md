...
3. **Serverautoritativ: Phase 1 vorbereitet, Umsetzung wartet auf Freigabe.**
   Entwurf mit Funktionsinventar, Zufallsquellen, State-/Event-Schnitt und
   Schattenbetrieb: [docs/SERVER-ENGINE-ENTWURF.md](docs/SERVER-ENGINE-ENTWURF.md).
   `js/engine/01-rng.js` delegiert regulär an Math.random; Seed und Ziehungsband
   dienen Tests. Der Host zeichnet Aktionen nur im RAM auf. Der Replay-Test
   nutzt die bestehende Browserlogik, noch keinen neuen Reducer.
   Phase 2 (gemeinsame Regeln und Schattenvergleich) und Phase 3 (Serverautorität)
   sind nicht freigegeben. Offene Entscheidungen stehen im Entwurf; insbesondere
   Vorschau/Latenz, Startdaten, Ausfallregel und Abnahmeschwelle. Datenbank,
   Edge Function, RPCs und Online-Schema bleiben in Phase 1 unverändert.
