...
const stages=trio.stageDefinitions();assert.equal(stages.length,10);assert.equal(stages[9].candidates.length,1);
assert.equal(stages[9].candidates[0].encounterId,'trio_helix_apex');
const seen=new Set();
for(const [i,s] of stages.entries()){
 const ids=new Set(s.candidates.map(o=>o.encounterId));assert(ids.size>=(i===9?1:3));
 for(const id of ids){assert(!seen.has(id));seen.add(id);}
 for(const option of s.candidates){const source=vm.runInContext(`trioEncounterById(${JSON.stringify(option.encounterId)})`,c);assert(source);const boss=vm.runInContext(`(()=>{const e=trioEncounterById(${JSON.stringify(option.encounterId)});return !!(e.isBoss||e.isMiniBoss||[4,9,14].includes(TRIO_CAMPAIGN_ENCOUNTERS.filter(x=>x.world===e.world).indexOf(e)));})()`,c);assert.equal(boss,[4,9].includes(i),'Bosse nur auf Stufe 5 und 10');assert.deepEqual(plain(option.enemies.map(e=>e.name)),plain(source.enemies.map(e=>e.name)),'vollständige Gegnergruppe');assert(option.enemies.every(e=>Number.isInteger(e.hp)&&e.hp>0));assert.equal(option.pressure,option.enemies.reduce((n,e)=>n+e.hp,0)*(1+.25*(option.enemies.length-1)));}
...
assert.equal(stages[9].candidates[0].enemies.length,4,'Helix Apex samt drei Seals');
