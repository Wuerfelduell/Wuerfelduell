...
r=fresh();for(let i=0;i<20;i++)assert(t.grant('a','gamble'));assert.equal(r.heroes.a.maxHp,10);assert.equal(r.heroes.a.perks.gamble,20);t.grant('a','constitution');assert.equal(r.heroes.a.maxHp,20);assert.equal(r.heroes.a.hp,20);t.grant('a','blood_pact');assert.equal(r.heroes.a.hp,20,'Blutpakt heilt nur bis zum Maximum, keine Ueberheilung mehr');
...
// Zweitfund: drei Stufen Laufzeit, je Stufe genau eine Kopie, ein erneuter Fund verlaengert.
r=fresh();t.grant('a','second_find');assert.equal(r.deferredRewards.length,0,'der Fund selbst kopiert nichts');
assert.equal(r.heroes.a.secondFindLeft,3,'drei Stufen Laufzeit');
t.grant('a','rest');assert.equal(r.deferredRewards.length,1);assert.equal(r.heroes.a.secondFindLeft,2);
assert(r.deferredRewards.every(x=>x.profileId==='b'&&x.dueStage===1&&x.from==='a'));
t.grant('a','damage');t.grant('a','damage');
assert.equal(r.deferredRewards.length,3);assert.equal(r.heroes.a.secondFindLeft,0);
t.grant('a','damage');assert.equal(r.deferredRewards.length,3,'nach drei Stufen ist Schluss');
t.grant('a','second_find');assert.equal(r.heroes.a.secondFindLeft,3,'ein erneuter Fund verlaengert');
...
assert.equal(r.deferredRewards.length,2,'die uebrigen ruecken eine Stufe nach');
assert(r.deferredRewards.every(x=>x.dueStage===2));
t.grant('b','rest',{copyReward:true});assert.equal(r.deferredRewards.length,2,'Kopien erzeugen keine Kopierkette');
