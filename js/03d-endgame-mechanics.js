...
function chooseHuntedHero(randomSource=window.WDRng.random){const alive=players.map((p,i)=>p?.campaignTeam==="hero"&&p.hp>0?i:null).filter(i=>i!=null);if(!alive.length)return null;const roll=Math.min(.999999,Math.max(0,Number(randomSource())||0));return alive[Math.floor(roll*alive.length)];}
