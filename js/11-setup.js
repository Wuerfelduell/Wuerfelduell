  function randomUniqueAbilityIds(count,exclude=[]){
    ...
    for(let i=pool.length-1;i>0;i--){const j=Math.floor(window.WDRng.random()*(i+1));[pool[i],pool[j]]=[pool[j],pool[i]];}
  ...
  function applyLocalModeSetup(){
    ...
    if(rules.id==="classic"){
      ...
      setupIntro.textContent="Mayhem ist ein reiner Local-Modus: nur echte Spielerprofile, keine Bots, maximal 6 Spieler. Alle Ability-Masteries laufen auf L2.";
