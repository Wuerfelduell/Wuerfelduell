  // Im Kampf entscheidet der Modus; auf der Karte müssen auch Start/Refresh routen.
  // Ausgenommen refreshButton: der Knopf gehört zum Bildschirm, nicht zum Modus.
  // js/15-app.js rendert beide Karten nacheinander, und dabei liefert der
  // Verteiler beide Male dasselbe Modul - der zweite Knopf bliebe stehen.
  Object.defineProperty(window,"WDBossRush",{get(){
    if(trioCampaignMode||(!campaignMode&&gameContext?.returnScreen==="trio"))return window.WDTrioBossRush;
    if(duoCampaignMode||(!campaignMode&&gameContext?.returnScreen==="duo"))return window.WDDuoBossRush;
    return null;
  }});
  function currentEncounterObject(){
    const rushEncounter=(duoCampaignMode||trioCampaignMode)?window.WDBossRush?.currentEncounter?.():null;
    return rushEncounter||(trioCampaignMode?trioEncounterById(trioCampaignEncounterId):(duoCampaignMode?duoEncounterById(duoCampaignEncounterId):campaignEncounterById(campaignEncounterId)));
  }
  function tx(s){ if(s==null||s==="") return s; try{ return window.t ? window.t(String(s)) : String(s); } catch{ return String(s); } }

  function resetEncounterRuntime(encounter){encounterRuntime={ruleIds:[...(ENCOUNTER_SPECIAL_RULES[encounter?.id]||[])],phaseRuleIds:[],phaseTriggered:false,phaseTriggeredIds:[],firstStrikeUsed:new Set(),armorUsed:new Set(),turnStarts:{},enemyTurnCount:0,lastHeroAttacker:null,markedHero:null,markTurns:0,markSource:null,escalationLevel:0,precisionUsed:new Set(),lastLightUsed:new Set(),arcaneUsed:new Set(),pendingOneShotDamage:{},worldMomentum:{}};}
  ...
  function soloCampaignHpBonusSlot(encounter){return encounter?.hpBonusSlot===3?3:2;}
  function campaignGrantedSecondAbility(encounter){
    const id=Number(encounter?.grantedSecondAbility);
    return REAL_ABILITY_IDS.includes(id)?id:null;
  }

  // V27.1: Challenge-relevante Fähigkeiten werden als Encounter-Ausrüstung gestellt.
  // Explizite Ability-Challenges sind eindeutig; HP-Kosten nutzen Loaded Dice/Blutpreis
  // und Heil-Challenges Lifesteal/Fähigkeit 12 als sinnvolle Standard-Werkzeuge.
  function campaignChallengeAbilityGroups(rule){
    const groups=[];
    const add=ids=>{const clean=[...new Set(ids.map(Number).filter(id=>REAL_ABILITY_IDS.includes(id)))];if(clean.length)groups.push(clean);};
    const walk=r=>{
      if(!r)return;
      if(r.type==="all"&&Array.isArray(r.rules))r.rules.forEach(walk);
      if(["ability_use","ability_success","each_hero_ability_use","each_hero_ability_success"].includes(r.type))add([r.ability]);
      else if(r.type==="twelve_triggers_min")add([22]);
      else if(["voluntary_hp_min","team_voluntary_hp_min","each_hero_voluntary_hp_min","self_damage_min","team_self_damage_min","each_hero_self_damage_min"].includes(r.type))add([18,11]);
      else if(["healed_min","team_healed_min","each_hero_healed_min"].includes(r.type))add([2,22]);
    };
    walk(rule);
    return groups;
  }
  function campaignChallengeGrantedAbility(encounter,already=[]){
    const owned=new Set((already||[]).map(Number).filter(Boolean));
    for(const group of campaignChallengeAbilityGroups(encounter?.challenge)){
      if(group.some(id=>owned.has(id)))continue;
      const pick=group.find(id=>REAL_ABILITY_IDS.includes(id));
      if(pick!=null)return pick;
    }
    return null;
  }
  function campaignChallengeDefaultAbility(encounter){
    return campaignChallengeGrantedAbility(encounter,[]);
  }
  function campaignPrimaryAbilityPool(ids,encounter){
    const standard=campaignChallengeDefaultAbility(encounter);
    const pool=(ids||[]).filter(id=>id!==standard);
    return pool.length?pool:[...(ids||[])];
  }
  ...
  function renderEncounterRuleBanner(){
    ...
    for(const mechanic of campaignMechanicSummary?.(enc)||[]){
      ...
      // Im mobilen Combat-HUD bleibt nur das aktive Label; die vollständige
  ...
  function campaignChallengeProgressInfo(c,heroIndex){
    ...
    switch(c.type){
      ...
      case "secondary_unlocked": label="Bonus-Fähigkeitswahl ausgelöst";value=players[heroIndex]?.campaignBonusDraftUsed?"ja":"nein";break;
      ...
      case "all_heroes_survive": {const alive=heroes.filter(i=>players[i]?.hp>0).length;label="Alle Helden überleben";value=`${alive} / ${heroes.length}`;failed=alive<heroes.length;break;}
      ...
      case "all_heroes_secondary_unlocked": label="Bonus-Draft bei allen";value=heroes.map((i,n)=>`P${n+1} ${players[i]?.campaignBonusDraftUsed?"✓":"–"}`).join(" · ");break;
  ...
  function checkBossPhase(targetIndex,beforeHp,afterHp){
    ...
    const enc=currentEncounterObject(),definition=bossPhaseFor(enc),target=players[targetIndex];
    if(!definition||!target||target.campaignTeam!=="enemy"||target.name!==definition.boss) return false;
    const phases=Array.isArray(definition.phases)?definition.phases:[definition];
    const maxHp=maxHpForPlayer(target);
    // Alle mit demselben Schadensereignis überschrittenen Schwellen werden vor
    // der Verarbeitung ermittelt. So kann Heilung in Phase I Phase II nicht
    // nachträglich verschlucken.
    const crossed=bossPhaseThresholdsCrossed(phases,encounterRuntime.phaseTriggeredIds,beforeHp,afterHp,maxHp).map(index=>({phase:phases[index],index}));
    if(!crossed.length)return false;

    // Ein tödlicher Phasen-Treffer startet zunächst alle fälligen Übergänge.
    // Der Boss bleibt für die definierte Transition am Leben; Phasenheilung
    // kann anschließend regulär greifen.
    if(target.hp<=0)target.hp=1;
    crossed.forEach(({phase,index:phaseIndex})=>{
      encounterRuntime.phaseTriggered=true;encounterRuntime.phaseTriggeredIds.push(phaseIndex);
      ...
      if(phase.aiProfile)target.aiProfile=phase.aiProfile;
      if(phase.mutator&&ELITE_MUTATORS[phase.mutator])target.mutatorId=phase.mutator;
      if(phase.markTurns){const hero=campaignHeroIndices().filter(i=>players[i]?.hp>0).sort((a,b)=>players[a].hp-players[b].hp)[0];if(hero!=null){encounterRuntime.markedHero=hero;encounterRuntime.markTurns=phase.markTurns;encounterRuntime.markSource="boss_phase";}}
      queueEventPopup(`${tx("PHASENWECHSEL")} · ${phase.title}`,"death");
      addLog(`${tx("PHASENWECHSEL")} – ${phase.title}: ${phase.desc}`);
    ...
    // Defensive Normalisierung schützt Achievement/UI vor doppelten IDs.
    encounterRuntime.phaseTriggeredIds=[...new Set(encounterRuntime.phaseTriggeredIds)];
  ...
  function markCampaignAbilityUse(index,abilityId,amount=1){
    ...
    window.WDMastery?.noteAbilityUse?.(index,abilityId);
  }
  ...
  function trioTeamKey(profile1OrId,profile2OrId,profile3OrId){
    const ids=[profile1OrId,profile2OrId,profile3OrId].map(v=>typeof v==="string"?v:v?.id).filter(Boolean).map(String);
    if(ids.length!==3||new Set(ids).size!==3) return null;
    return ids.sort().join("::");
  }
  function trioCampaignProgress(profile1,profile2,profile3,create=true){
    const key=trioTeamKey(profile1,profile2,profile3);if(!key)return null;
    if(!saveData.trioCampaigns||typeof saveData.trioCampaigns!=="object"||Array.isArray(saveData.trioCampaigns))saveData.trioCampaigns={};
    let progress=saveData.trioCampaigns[key];if(!progress&&create){progress=defaultTrioProgress(profile1.id,profile2.id,profile3.id);saveData.trioCampaigns[key]=progress;}if(!progress)return null;
    progress.profileIds=[profile1.id,profile2.id,profile3.id].sort();progress.completedEncounters=Array.isArray(progress.completedEncounters)?[...new Set(progress.completedEncounters.map(String).filter(Boolean))]:[];progress.wins=Math.max(0,Math.floor(Number(progress.wins)||0));progress.losses=Math.max(0,Math.floor(Number(progress.losses)||0));progress.campaignVersion=Math.max(Number(progress.campaignVersion)||1,CAMPAIGN_VERSION);return progress;
  }
  function trioEncounterById(id){return TRIO_CAMPAIGN_ENCOUNTERS.find(e=>e.id===id)||null;}
  function trioWorldById(id){return TRIO_CAMPAIGN_WORLDS.find(w=>w.id===id)||null;}
  function trioEncountersForWorld(worldId){return TRIO_CAMPAIGN_ENCOUNTERS.filter(e=>(e.world||"trinity")===worldId);}
  function trioWorldUnlocked(profile1,profile2,profile3,worldOrId){
    const world=typeof worldOrId==="string"?trioWorldById(worldOrId):worldOrId;
    if(!profile1||!profile2||!profile3||new Set([profile1.id,profile2.id,profile3.id]).size!==3||!world) return false;
    const progress=trioCampaignProgress(profile1,profile2,profile3);
    return (world.unlockRequires||[]).every(id=>progress.completedEncounters.includes(id));
  }
  function isTrioWorldComplete(profile1,profile2,profile3,worldId){
    const world=trioWorldById(worldId),progress=trioCampaignProgress(profile1,profile2,profile3,false);
    return !!world&&!!progress&&progress.completedEncounters.includes(world.finalEncounterId);
  }
  function trioEncounterAvailable(profile1,profile2,profile3,encounter){if(!profile1||!profile2||!profile3||new Set([profile1.id,profile2.id,profile3.id]).size!==3||!encounter)return false;if(!trioWorldUnlocked(profile1,profile2,profile3,encounter.world||"trinity"))return false;const progress=trioCampaignProgress(profile1,profile2,profile3);if(progress.completedEncounters.includes(encounter.id))return true;return (encounter.requires||[]).every(id=>progress.completedEncounters.includes(id));}
  function defaultTrioEncounter(profile1,profile2,profile3,worldId=trioWorldId){
    const world=trioWorldById(worldId)||TRIO_CAMPAIGN_WORLDS[0];
    const worldEncounters=trioEncountersForWorld(world.id);
    const progress=trioCampaignProgress(profile1,profile2,profile3,false);
    if(!progress) return worldEncounters[0]||null;
    return worldEncounters.find(e=>!progress.completedEncounters.includes(e.id)&&trioEncounterAvailable(profile1,profile2,profile3,e))
      || (progress.completedEncounters.includes(world.finalEncounterId)?trioEncounterById(world.finalEncounterId):worldEncounters[0]);
  }
  function nextTrioCampaignEncounter(profile1,profile2,profile3,currentEncounter){
    if(!profile1||!profile2||!profile3||!currentEncounter)return null;
    const progress=trioCampaignProgress(profile1,profile2,profile3,false),done=new Set(progress?.completedEncounters||[]);
    const worldIndex=TRIO_CAMPAIGN_WORLDS.findIndex(w=>w.id===(currentEncounter.world||"trinity"));
    for(let wi=Math.max(0,worldIndex);wi<TRIO_CAMPAIGN_WORLDS.length;wi++){
      const world=TRIO_CAMPAIGN_WORLDS[wi];
      if(!trioWorldUnlocked(profile1,profile2,profile3,world))continue;
      const list=trioEncountersForWorld(world.id);
      let start=0;
      if(wi===worldIndex){
        const ci=list.findIndex(e=>e.id===currentEncounter.id);
        start=Math.max(0,ci+1);
      }
      for(let i=start;i<list.length;i++){
        const e=list[i];
        if(!done.has(e.id)&&trioEncounterAvailable(profile1,profile2,profile3,e))return e;
      }
    }
    return null;
  }
  ...
  function nextSoloCampaignEncounter(profile,currentEncounter){
    if(!profile||!currentEncounter)return null;
    const progress=campaignProgress(profile),done=new Set(progress?.completedEncounters||[]);
    const worldIndex=CAMPAIGN_WORLDS.findIndex(w=>w.id===(currentEncounter.world||"house"));
    for(let wi=Math.max(0,worldIndex);wi<CAMPAIGN_WORLDS.length;wi++){
      const world=CAMPAIGN_WORLDS[wi];
      if(!campaignWorldUnlocked(profile,world))continue;
      const list=campaignEncountersForWorld(world.id);
      let start=0;
      if(wi===worldIndex){
        const ci=list.findIndex(e=>e.id===currentEncounter.id);
        start=Math.max(0,ci+1);
      ...
      for(let i=start;i<list.length;i++){
        const e=list[i];
        if(!done.has(e.id)&&campaignEncounterAvailable(profile,e))return e;
      }
    }
    return null;
  }
  function nextDuoCampaignEncounter(profile1,profile2,currentEncounter){
    if(!profile1||!profile2||!currentEncounter)return null;
    const progress=duoCampaignProgress(profile1,profile2,false),done=new Set(progress?.completedEncounters||[]);
    const worldIndex=DUO_CAMPAIGN_WORLDS.findIndex(w=>w.id===(currentEncounter.world||"covenant"));
    for(let wi=Math.max(0,worldIndex);wi<DUO_CAMPAIGN_WORLDS.length;wi++){
      const world=DUO_CAMPAIGN_WORLDS[wi];
      if(!duoWorldUnlocked(profile1,profile2,world))continue;
      const list=duoEncountersForWorld(world.id);
      let start=0;
      if(wi===worldIndex){
        const ci=list.findIndex(e=>e.id===currentEncounter.id);
        start=Math.max(0,ci+1);
      ...
      for(let i=start;i<list.length;i++){
        const e=list[i];
        if(!done.has(e.id)&&duoEncounterAvailable(profile1,profile2,e))return e;
      }
    }
    return null;
  }
  ...
  function renderCampaign(){
    ...
    if(grantedSecondForSelected!=null){
      const grantedOpt=[...campaignAbilitySelect.options].find(opt=>Number(opt.value)===grantedSecondForSelected);
      if(grantedOpt){
        grantedOpt.disabled=true;
        grantedOpt.textContent=`${grantedSecondForSelected} · ${ABILITIES[grantedSecondForSelected].name} · wird gestellt`;
      }
      if(Number(campaignAbilitySelect.value)===grantedSecondForSelected){
        const fallback=unlocked.find(id=>id!==grantedSecondForSelected);
        if(fallback!=null) campaignAbilitySelect.value=String(fallback);
      }
    }
    const challengeStandardForSelected=campaignChallengeDefaultAbility(selectedEncounter);
    const selectedAssistPair=campaignAssistAbilityPair(selectedEncounter);
    if(challengeStandardForSelected!=null&&challengeStandardForSelected!==grantedSecondForSelected&&!selectedAssistPair?.includes(challengeStandardForSelected)){
      const challengeOpt=[...campaignAbilitySelect.options].find(opt=>Number(opt.value)===challengeStandardForSelected);
      if(challengeOpt){
        challengeOpt.disabled=true;
        challengeOpt.textContent=`${challengeStandardForSelected} · ${ABILITIES[challengeStandardForSelected].name} · Challenge-Ausrüstung`;
      }
      if(Number(campaignAbilitySelect.value)===challengeStandardForSelected){
        const fallback=unlocked.find(id=>id!==challengeStandardForSelected&&id!==grantedSecondForSelected);
        if(fallback!=null) campaignAbilitySelect.value=String(fallback);
      }
    }
    ...
    campaignAbilityGrid.innerHTML=REAL_ABILITY_IDS.map(id=>{
      ...
      const masteryThreshold=window.WDMastery?.abilityThreshold?.(profile,"solo",selectedEncounter)??soloCampaignHpBonusThreshold(selectedEncounter);
  ...
  function renderDuoCampaign(){
    ...
    const duoPrimaryPool=campaignPrimaryAbilityPool(sharedDuoAbilityPool,encounter);
    fillAbilities(duoAbility1Select,duoPrimaryPool);fillAbilities(duoAbility2Select,duoPrimaryPool);
  ...
  function renderTrioCampaign(){
    ...
    if(validTeam&&!trioWorldUnlocked(p1,p2,p3,trioWorldId)){
      trioWorldId=TRIO_CAMPAIGN_WORLDS.find(w=>trioWorldUnlocked(p1,p2,p3,w))?.id||"trinity";trioCampaignEncounterId=null;
    }
    const world=trioWorldById(trioWorldId)||TRIO_CAMPAIGN_WORLDS[0];
    const worldUnlocked=validTeam&&trioWorldUnlocked(p1,p2,p3,world);
    const worldEncounters=trioEncountersForWorld(world.id);
    const completed=progress?worldEncounters.filter(e=>progress.completedEncounters.includes(e.id)).length:0;
    const worldComplete=validTeam&&isTrioWorldComplete(p1,p2,p3,world.id);
    if(trioCampaignWorldTabs){
      trioCampaignWorldTabs.innerHTML=TRIO_CAMPAIGN_WORLDS.map((w,wi)=>{
        const unlocked=validTeam&&trioWorldUnlocked(p1,p2,p3,w),done=unlocked&&isTrioWorldComplete(p1,p2,p3,w.id),active=w.id===world.id;
    ...
    trioCampaignStartBtn.disabled=!validTeam||!worldUnlocked||!encounter||!trioEncounterAvailable(p1,p2,p3,encounter)||!primaryOk;trioCampaignStartBtn.title=!primaryOk?"Für diesen Encounter gilt das angezeigte Pflicht-Loadout.":"";window.WDMastery?.refreshAll?.();window.WDTrioBossRush?.refreshButton?.();
  }
  function openTrioCampaignScreen(){clearBotAutomation();tutorialMode=false;campaignMode=false;duoCampaignMode=false;trioCampaignMode=false;campaignMetrics=freshCampaignMetrics();document.body.classList.remove("playing","bot-acting");game.classList.add("hidden");game.classList.remove("campaign-game","trio-game");winnerBox.classList.add("hidden");nextRoundBox.classList.add("hidden");gameContext={mode:"trio-campaign-map",returnScreen:"trio",profileIds:[trioProfile1Id,trioProfile2Id,trioProfile3Id].filter(Boolean),encounterId:trioCampaignEncounterId};renderTrioCampaign();openFrontScreen(trioCampaignScreen);}
  function returnToTrioCampaignMap(){window.WDBossRush?.reset?.({restoreSelection:true});clearBotAutomation();tutorialMode=false;campaignMode=false;duoCampaignMode=false;trioCampaignMode=false;campaignMetrics=freshCampaignMetrics();isAnimating=false;eventPopupQueue=[];eventPopupBusy=false;eventPopup.classList.remove("active","death","win","survive");quitModal.classList.add("hidden");winnerBox.classList.add("hidden");nextRoundBox.classList.add("hidden");document.body.classList.remove("playing","bot-acting");game.classList.add("hidden");game.classList.remove("campaign-game","trio-game");rotatingBoard.style.transform="rotate(0deg) scale(1)";restartBtn.disabled=false;if(trioCampaignEncounterId&&trioEncounterById(trioCampaignEncounterId)) trioWorldId=trioEncounterById(trioCampaignEncounterId).world||"trinity";gameContext={mode:"trio-campaign-map",returnScreen:"trio",profileIds:[trioProfile1Id,trioProfile2Id,trioProfile3Id].filter(Boolean),encounterId:trioCampaignEncounterId};renderTrioCampaign();hideFrontScreens();trioCampaignScreen.classList.remove("hidden");window.scrollTo?.(0,0);}

  function openDuoCampaignScreen(){
    clearBotAutomation();tutorialMode=false;campaignMode=false;duoCampaignMode=false;trioCampaignMode=false;campaignMetrics=freshCampaignMetrics();
  ...
  function returnToDuoCampaignMap(){
    window.WDBossRush?.reset?.({restoreSelection:true});
    clearBotAutomation();tutorialMode=false;campaignMode=false;duoCampaignMode=false;trioCampaignMode=false;campaignMetrics=freshCampaignMetrics();isAnimating=false;
  ...
  function campaignChallengeRuleMet(c,heroIndex){
    ...
    if(c.type==="secondary_unlocked") return !!players[heroIndex]?.campaignBonusDraftUsed;
    ...
    if(c.type==="all_heroes_secondary_unlocked"){const ids=players.map((p,i)=>p.campaignTeam==="hero"?i:null).filter(i=>i!=null);return ids.length>=2&&ids.every(i=>!!players[i]?.campaignBonusDraftUsed);}
  ...
  function awardCampaignTrophy(profile,encounter){
    ...
    awardEndgameEncounterAchievements(encounter,true,true);
  ...
  function awardTrioCampaignTrophies(profile1,profile2,profile3,encounter,newlyCompleted){if(!profile1||!profile2||!profile3||!encounter)return 0;awardEndgameEncounterAchievements(encounter,true,true);if(!newlyCompleted&&!encounter.farmTrophy)return 0;[profile1,profile2,profile3].forEach(profile=>{const progress=campaignProgress(profile);progress.trophies=Math.max(0,Math.floor(Number(progress.trophies)||0))+1;});return 1;}
  function startTrioCampaignEncounter(options={}){
    const bossRush=!!options?.bossRush&&!!window.WDBossRush?.isActive?.();
    const p1=getProfile(trioProfile1Select.value),p2=getProfile(trioProfile2Select.value),p3=getProfile(trioProfile3Select.value),encounter=bossRush?window.WDBossRush.currentEncounter():trioEncounterById(trioCampaignEncounterId);if(!p1||!p2||!p3||new Set([p1.id,p2.id,p3.id]).size!==3||!encounter||(!bossRush&&!trioEncounterAvailable(p1,p2,p3,encounter)))return false;
    trioCampaignEncounterId=encounter.id;
    let selected=[+trioAbility1Select.value,+trioAbility2Select.value,+trioAbility3Select.value].map(id=>CHOOSABLE_ABILITY_IDS.includes(id)?id:(CHOOSABLE_ABILITY_IDS[0]||3));const required=Array.isArray(encounter.requiredPrimaryAbilities)?encounter.requiredPrimaryAbilities.map(Number):null;const trioChallengeStandard=campaignChallengeDefaultAbility(encounter);if((!required||required.length!==3)&&trioChallengeStandard!=null)selected=selected.map(id=>id===trioChallengeStandard?(CHOOSABLE_ABILITY_IDS.find(x=>x!==trioChallengeStandard)??id):id);if(required&&required.length===3&&!required.every((id,i)=>selected[i]===id)){renderTrioCampaign();return;}
    clearBotAutomation();resetTutorialUi();tutorialMode=false;campaignMode=true;duoCampaignMode=false;trioCampaignMode=true;campaignProfileId=null;trioProfile1Id=p1.id;trioProfile2Id=p2.id;trioProfile3Id=p3.id;gameContext={mode:bossRush?"trio-boss-rush-game":"trio-campaign-game",returnScreen:"trio",profileIds:[p1.id,p2.id,p3.id],encounterId:encounter.id};campaignMetrics=freshCampaignMetrics();resetEncounterRuntime(encounter);
    const grants=Array.isArray(encounter.grantedThirdAbilities)?encounter.grantedThirdAbilities.map(Number):[];
    const pickRandomTrioStartAbility=blocked=>{
    ...
    const makeHero=(profile,ability,pos)=>{
      ...
      let second=campaignChallengeGrantedAbility(encounter,[ability,third]);
      if(encounter.randomStartSecondAbility){
        const randomId=pickRandomTrioStartAbility([ability,second,third]);
        if(randomId!=null){
          if(second==null) second=randomId;
          else if(third==null) third=randomId;
        }
      }
      const heroStartHp=(bossRush?Math.max(1,Number(encounter.playerHp)||START_HP):START_HP)+(window.WDMastery?.hpBonus?.(profile,"trio",encounter)||0);
      const vitals=bossRush?window.WDBossRush.startingVitals(profile,heroStartHp):{hp:heroStartHp,maxHp:heroStartHp};
      const loadout=bossRush?window.WDBossRush.startingLoadout(profile,ability,second):{secondAbility:second,thirdAbility:third,secondAbilityUnlocked:second!=null,thirdAbilityUnlocked:third!=null,campaignBonusDraftUsed:false};
      return {name:profile.name,battleTag:`#${profile.tagNumber}`,profileId:profile.id,botLevel:"human",campaignTeam:"hero",hp:vitals.hp,maxHp:vitals.maxHp,campaignStartHp:vitals.maxHp,ability,secondAbility:loadout.secondAbility,thirdAbility:loadout.thirdAbility,fourthAbility:null,secondAbilityUnlocked:loadout.secondAbilityUnlocked,thirdAbilityUnlocked:loadout.thirdAbilityUnlocked,fourthAbilityUnlocked:false,campaignBonusDraftUsed:loadout.campaignBonusDraftUsed,rolledAbility:"TRIO",primaryWasChosen:true,secondAbilityWasChosen:false,thirdAbilityWasChosen:false,fourthAbilityWasChosen:false,seat:0,diceDesign:profile.selectedDice||"classic",...playerCosmeticsFromProfile(profile),wins:0,momentumStreak:0,lastStandUsed:false,roundLastStandTriggered:false,damageSinceLastOwnTurn:false,bloodRushPrimed:false,voluntaryHpPaidThisTurn:false,botBloodUsesThisAttack:0};
    };const heroes=[makeHero(p1,selected[0],0),makeHero(p2,selected[1],1),makeHero(p3,selected[2],2)];const enemies=encounter.enemies.map((enemy,i)=>buildCampaignEnemyPlayer(enemy,encounter,i));players=[];const max=Math.max(heroes.length,enemies.length);for(let i=0;i<max;i++){if(heroes[i])players.push(heroes[i]);if(enemies[i])players.push(enemies[i]);}initializeEncounterMechanics();
    resetRoundStats();current=bossRush?players.findIndex(player=>player.campaignTeam==="hero"&&player.hp>0):0;if(current<0)current=0;prepareBloodRushForTurn(current);applyEncounterTurnStartRule(current);dice=freshDice();phase="idle";isAnimating=false;attackFace=null;attackTarget=null;pendingCampaignAttackStart=null;attackHits=0;attackDamage=0;firstAttackRoll=true;currentAttackRollNewHits=0;baseRerollUsed=false;luckRerollIndex=null;luckRerollSecondUsed=false;luckRerollUses=0;loadedDiceUsed=false;loadedDiceUses=0;lastBaseRollIndices=[];attackPowerUsed=false;attackPowerUses=0;precisionUses=0;momentumBonus=0;bloodPriceNeighbors=[];bloodPricePaidThisRoll=0;bloodPriceWasPreActivatedThisRoll=false;bloodRushActiveThisAttack=false;doubleTapApplied=false;lastAttackRollIndices=[];attackRollCount=0;attackMasteryRollCount=0;normalAttackHitsThisAttack=0;exactFaceHitsThisAttack=0;wildcardAttackHitsThisAttack=0;wildcardTriggeredThisAttack=false;masteryL2AttackBonusesApplied=false;wildcardSecondRollArmed=false;currentAttackSource="normal";currentAttackBaseTotal=null;gamblingRetryUsed=false;gamblingRetryPending=false;pendingDamage=null;pendingHeal=null;pendingExtraDamageFx=[];pendingExtraHealFx=[];roundNumber=bossRush?window.WDBossRush.stageNumber():Math.max(1,trioEncountersForWorld(encounter.world||"trinity").findIndex(e=>e.id===encounter.id)+1);roundEliminationOrder=[];lastPlaceIndex=null;roundWinnerHandled=false;roundWinnerIndex=null;nextRoundAbilityRolls=[];eventPopupQueue=[];eventPopupBusy=false;secondAbilityDraftBusy=false;secondAbilityDraftIndex=null;deferredBaseAdvance=false;gamblingRolling=false;gamblingBaseTotal=null;gamblingRetryActions?.classList.add("hidden");gamblingModal.classList.add("hidden");highStakesRolling=false;highStakesDecisionThisAttack=false;highStakesModal.classList.add("hidden");perfect25Rolling=false;perfect25D4Rolling=false;perfect25BaseTotal=null;pendingPerfect25Total=null;perfect25Modal.classList.add("hidden");perfect25D4Modal.classList.add("hidden");insuranceRolling=false;insuranceContext=null;insuranceModal.classList.add("hidden");counterRolling=false;counterContext=null;counterDiceState=[];counterHits=0;counterFirstRoll=true;pendingCounterattack=null;deferredAttackFinish=false;counterModal.classList.add("hidden");wildcardFace=null;secondAbilityDraftQueue=[];secondAbilityModal.classList.add("hidden");
    logEl.innerHTML="";winnerBox.classList.add("hidden");nextRoundBox.classList.add("hidden");nextRoundPrepBtn.classList.add("hidden");restartBtn.textContent=bossRush?"Boss Rush beenden":"Zur Trio-Kampagne";hideFrontScreens();game.classList.remove("hidden");game.classList.add("campaign-game","trio-game");game.classList.toggle("boss-rush-game",bossRush);document.body.classList.add("playing");window.scrollTo?.(0,0);if(bossRush)addLog(`Boss Rush ${roundNumber}/${window.WDBossRush?.stageCount?.()??10}: ${encounter.title}. Verbleibende HP und gewählte Rush-Fähigkeiten werden in den nächsten Kampf übernommen.`);else addLog(`💪 Trio-Kampagne: ${encounter.title} – ${encounter.subtitle}. Challenge: ${encounter.challenge.text}`);const trioChallengeAbility=campaignChallengeGrantedAbility(encounter,[]);if(trioChallengeAbility!=null)addLog(`🎯 Challenge-Ausrüstung: ${ABILITIES[trioChallengeAbility].name} wird automatisch mitgegeben, sofern das Encounter-Loadout sie nicht bereits enthält.`);if(bossRush)addLog(`Boss-Rush-Draft: Die erste Bonuswahl füllt die 2. Fähigkeit. Nach jeder Stufe stehen drei Belohnungen bereit; Ability-Angebote können die 3. Fähigkeit für die nächsten Kämpfe neu ausrichten.`);else addLog(`✨ Bonus-Draft pro Spieler: erster eigener Gegner-Kill ODER ≤15 HP – was zuerst passiert.`);if(encounter.randomStartSecondAbility)addLog(`🎲 Zufalls-Start: Jeder startet mit einer zusätzlichen zufälligen 2. Fähigkeit. Der Trigger füllt die 3.`);encounterRuleText(encounter).forEach(r=>addLog(`⚙ Sonderregel ${r.name}: ${r.desc}`));if(bossPhaseFor(encounter))addLog(`👹 Bossphase vorbereitet: ${bossPhaseFor(encounter).title} bei ${Math.round((bossPhaseFor(encounter).threshold||.5)*100)} % Boss-HP.`);const extra=heroes.map(h=>[h.secondAbility,h.thirdAbility].filter(x=>x!=null).map(id=>ABILITIES[id].name).join(" + ")).map((txt,i)=>txt?`${heroes[i].name}: ${txt}`:null).filter(Boolean);if(extra.length)addLog(`🎁 Encounter-Fähigkeiten: ${extra.join(" · ")}.`);addLog(`💀 Verloren ist erst, wenn alle drei Helden gefallen sind.`);renderAll();return true;
  }
  function finishTrioCampaignEncounter(heroWon){if(window.WDBossRush?.isActive?.())return window.WDBossRush.finishEncounter(heroWon);if(roundWinnerHandled)return true;roundWinnerHandled=true;const encounter=trioEncounterById(trioCampaignEncounterId),p1=getProfile(trioProfile1Id),p2=getProfile(trioProfile2Id),p3=getProfile(trioProfile3Id),progress=trioCampaignProgress(p1,p2,p3),heroIndices=campaignHeroIndices(),challengeMet=heroWon&&campaignChallengeMet(encounter,heroIndices[0]??0);roundWinnerIndex=heroWon?(heroIndices.find(i=>players[i].hp>0)??heroIndices[0]):players.findIndex(p=>p.hp>0&&p.campaignTeam==="enemy");if(heroWon){progress.wins++;heroIndices.filter(i=>players[i].hp>0).forEach(i=>{players[i].wins=(players[i].wins||0)+1;checkRoundWinnerAchievements(i);});}else progress.losses++;heroIndices.forEach(i=>window.WDMastery?.noteMatchEnd?.(i,heroWon));let newlyCompleted=false,trophiesEach=0,masteryXpEach=0;if(heroWon&&challengeMet&&!progress.completedEncounters.includes(encounter.id)){progress.completedEncounters.push(encounter.id);newlyCompleted=true;}if(heroWon&&challengeMet){trophiesEach=awardTrioCampaignTrophies(p1,p2,p3,encounter,newlyCompleted);[p1,p2,p3].forEach(p=>window.WDShop?.verdiene?.(p,newlyCompleted?(TRIO_CAMPAIGN_WORLDS.some(w=>w.finalEncounterId===encounter.id)?"weltboss_erst":"encounter_erst"):"encounter_wieder"));const xs=[p1,p2,p3].map(p=>window.WDMastery?.awardXp?.(p,"trio",encounter,newlyCompleted)||0);masteryXpEach=Math.max(...xs);}progress.campaignVersion=Math.max(Number(progress.campaignVersion)||1,CAMPAIGN_VERSION);saveGameData();window.WDMastery?.refreshAll?.();if(newlyCompleted){const next=nextTrioCampaignEncounter(p1,p2,p3,encounter)||defaultTrioEncounter(p1,p2,p3,encounter.world||"trinity");if(next){trioCampaignEncounterId=next.id;trioWorldId=next.world||"trinity";}}const encounterWorld=encounter.world||"trinity";const world=trioWorldById(encounterWorld),complete=!!world&&progress.completedEncounters.includes(world.finalEncounterId);const worldIndex=TRIO_CAMPAIGN_WORLDS.findIndex(w=>w.id===encounterWorld),nextWorld=worldIndex>=0?TRIO_CAMPAIGN_WORLDS[worldIndex+1]:null;const nextWorldUnlocked=!!(newlyCompleted&&nextWorld&&trioWorldUnlocked(p1,p2,p3,nextWorld));const nextWorldText=nextWorldUnlocked?`<br>🌐 <strong>${escapeHtml(nextWorld.name)} wurde freigeschaltet!</strong>`:"";if(heroWon&&challengeMet){const masteryText=masteryXpEach?`<br>⭐ <strong>+${masteryXpEach} Trio-Mastery XP für jedes Profil</strong>`:"";const trophyText=trophiesEach?`<br>🏆 <strong>+${trophiesEach} Trophäe für jedes Profil</strong> · ${escapeHtml(p1.name)} ${campaignProgress(p1).trophies} · ${escapeHtml(p2.name)} ${campaignProgress(p2).trophies} · ${escapeHtml(p3.name)} ${campaignProgress(p3).trophies}`:"";queueEventPopup(complete?"Trio-Welt geschafft!":"Trio-Encounter geschafft!","win");winnerText.innerHTML=complete?"🏆 TRIO-WELT GESCHAFFT!":"💪 Trio-Encounter geschafft!";roundResultText.innerHTML=`✅ Gegnerteam besiegt.<br>✅ Challenge erfüllt: <strong>${escapeHtml(encounter.challenge.text)}</strong>${trophyText}${window.WDShop?.gutschriftText?.(escapeHtml)||""}${masteryText}${newlyCompleted?"<br>Der nächste Trio-Encounter wurde freigeschaltet.":"<br>Dieser Trio-Encounter war bereits abgeschlossen."}${nextWorldText}`;}else if(heroWon){queueEventPopup("Challenge verfehlt","survive");winnerText.textContent="⚠️ Sieg – aber Challenge verfehlt";roundResultText.innerHTML=`Ihr habt das Gegnerteam besiegt, aber die Pflichtaufgabe fehlt:<br><strong>${escapeHtml(encounter.challenge.text)}</strong><br>Der nächste Trio-Encounter bleibt gesperrt.`;}else{queueEventPopup("Trio-Encounter verloren","death");winnerText.textContent="💀 Trio-Encounter verloren";roundResultText.innerHTML=`Das Gegnerteam hat alle drei Helden ausgeschaltet. Euer Trio-Fortschritt bleibt gespeichert.`;}roundStandings.innerHTML=players.map(p=>`<div class="round-score-row${p.campaignTeam==="hero"&&heroWon?" winner-row":""}"><div class="round-score-name">${escapeHtml(p.name)}</div><div class="round-score-meta">${p.campaignTeam==="hero"?"💪 Trio-Spieler":"🤖 Gegner"} · ${Math.max(0,p.hp)} HP</div></div>`).join("");renderRoundStats();clearBotAutomation();winnerBox.classList.remove("hidden");nextRoundBox.classList.add("hidden");nextRoundPrepBtn.classList.add("hidden");restartBtn.textContent="Zur Trio-Kampagne";restartBtn.disabled=false;gameContext.returnScreen="trio";gameContext.encounterId=trioCampaignEncounterId;turnLine.textContent="Trio-Encounter beendet";statusEl.textContent="";abilityState.innerHTML="";hideAllControls();renderPlayers();roundNumberEl.textContent=roundNumber;return true;}

  function startDuoCampaignEncounter(options={}){
    const bossRush=!!options?.bossRush&&!!window.WDBossRush?.isActive?.();
    const p1=getProfile(duoProfile1Select.value),p2=getProfile(duoProfile2Select.value),encounter=bossRush?window.WDBossRush.currentEncounter():duoEncounterById(duoCampaignEncounterId);
    if(!p1||!p2||p1.id===p2.id||!encounter||(!bossRush&&!duoEncounterAvailable(p1,p2,encounter))) return false;
    ...
    if(challengeStandard!=null&&a1===challengeStandard)a1=sharedDuoAbilityPool.find(id=>id!==challengeStandard)??a1;
    if(challengeStandard!=null&&a2===challengeStandard)a2=sharedDuoAbilityPool.find(id=>id!==challengeStandard)??a2;
    ...
    const makeHero=(profile,ability)=>{
      ...
      const vitals=bossRush?window.WDBossRush.startingVitals(profile,normalStartHp):{hp:normalStartHp,maxHp:normalStartHp};
      const loadout=bossRush
        ? window.WDBossRush.startingLoadout(profile,ability,challengeAbility)
    ...
    const enemies=encounter.enemies.map((enemy,i)=>buildCampaignEnemyPlayer(enemy,encounter,i));
    players=[];
    const max=Math.max(heroes.length,enemies.length);for(let i=0;i<max;i++){if(heroes[i])players.push(heroes[i]);if(enemies[i])players.push(enemies[i]);}initializeEncounterMechanics();
    ...
    roundNumber=bossRush?window.WDBossRush.stageNumber():Math.max(1,duoEncountersForWorld(encounter.world||"covenant").findIndex(e=>e.id===encounter.id)+1);roundEliminationOrder=[];lastPlaceIndex=null;roundWinnerHandled=false;roundWinnerIndex=null;nextRoundAbilityRolls=[];eventPopupQueue=[];eventPopupBusy=false;secondAbilityDraftBusy=false;secondAbilityDraftIndex=null;deferredBaseAdvance=false;
    ...
    if(bossRush)addLog(`Boss Rush ${roundNumber}/${window.WDBossRush?.stageCount?.()??10}: ${encounter.title}. Verbleibende HP und gewählte Rush-Fähigkeiten werden in den nächsten Kampf übernommen.`);
    ...
    if(bossRush)addLog(`Boss-Rush-Draft: Die erste Bonuswahl füllt die 2. Fähigkeit. Nach jeder Stufe stehen drei Belohnungen bereit; Ability-Angebote können die 3. Fähigkeit für die nächsten Kämpfe neu ausrichten.`);
  ...
  function finishDuoCampaignEncounter(heroWon){
    if(window.WDBossRush?.isActive?.())return window.WDBossRush.finishEncounter(heroWon);
    ...
    heroIndices.forEach(i=>window.WDMastery?.noteMatchEnd?.(i,heroWon));
    let newlyCompleted=false,trophiesEach=0,masteryXpEach=0;if(heroWon&&challengeMet&&!progress.completedEncounters.includes(encounter.id)){progress.completedEncounters.push(encounter.id);newlyCompleted=true;}if(heroWon&&challengeMet){trophiesEach=awardDuoCampaignTrophies(p1,p2,encounter,newlyCompleted);[p1,p2].forEach(p=>window.WDShop?.verdiene?.(p,newlyCompleted?(isDuoWorldBossEncounter(encounter)?"weltboss_erst":"encounter_erst"):"encounter_wieder"));const x1=window.WDMastery?.awardXp?.(p1,"duo",encounter,newlyCompleted)||0;const x2=window.WDMastery?.awardXp?.(p2,"duo",encounter,newlyCompleted)||0;masteryXpEach=Math.min(x1,x2)||Math.max(x1,x2);}progress.campaignVersion=Math.max(Number(progress.campaignVersion)||1,CAMPAIGN_VERSION);saveGameData();window.WDMastery?.refreshAll?.();
    const encounterWorld=encounter.world||"covenant";duoWorldId=encounterWorld;
    if(newlyCompleted){const next=nextDuoCampaignEncounter(p1,p2,encounter)||defaultDuoEncounter(p1,p2,encounterWorld);if(next){duoCampaignEncounterId=next.id;duoWorldId=next.world||encounterWorld;}}
    ...
    if(heroWon&&challengeMet){const masteryText=masteryXpEach?`<br>⭐ <strong>+${masteryXpEach} Duo-Mastery XP für jedes Profil</strong>`:"";const trophyText=trophiesEach?`<br>🏆 <strong>+${trophiesEach} Trophäe für jedes Profil</strong> · ${escapeHtml(p1.name)}: ${campaignProgress(p1).trophies} · ${escapeHtml(p2.name)}: ${campaignProgress(p2).trophies}`:"";queueEventPopup(complete?"Duo-Welt geschafft!":"Duo-Encounter geschafft!","win");winnerText.innerHTML=complete?"🏆 DUO-WELT GESCHAFFT!":"🤝 Duo-Encounter geschafft!";roundResultText.innerHTML=`✅ Gegnerteam besiegt.<br>✅ Challenge erfüllt: <strong>${escapeHtml(encounter.challenge.text)}</strong>${trophyText}${window.WDShop?.gutschriftText?.(escapeHtml)||""}${masteryText}${newlyCompleted?"<br>Der nächste Duo-Encounter wurde freigeschaltet.":"<br>Dieser Duo-Encounter war bereits abgeschlossen."}${nextWorldText}`;}
    ...
    roundStandings.innerHTML=players.map((p,i)=>`<div class="round-score-row${p.campaignTeam==="hero"&&heroWon?" winner-row":""}"><div class="round-score-name">${escapeHtml(p.name)}</div><div class="round-score-meta">${p.campaignTeam==="hero"?"🤝 Duo-Spieler":"🤖 Gegner"} · ${Math.max(0,p.hp)} HP</div></div>`).join("");
  ...
  function startCampaignEncounter(){
    ...
    if(challengeStandard!=null && heroAbility===challengeStandard && !assistPair?.includes(challengeStandard)){
      const fallback=unlocked.find(id=>id!==challengeStandard&&(!assistPair||assistPair.includes(id)));
      if(fallback!=null)heroAbility=fallback;
    }
    if(assistPair&&!assistPair.includes(heroAbility)){
      renderCampaign();
      return;
    }
    const grantedSecondAbility=campaignGrantedSecondAbility(encounter);
    if(grantedSecondAbility!=null && heroAbility===grantedSecondAbility){
      heroAbility=unlocked.find(id=>id!==grantedSecondAbility&&(!assistPair||assistPair.includes(id)))??heroAbility;
    }
    const grantedThirdAbility=campaignAssistThirdAbility(encounter,heroAbility);
    const challengeGrantedAbility=campaignChallengeGrantedAbility(encounter,[heroAbility,grantedSecondAbility,grantedThirdAbility]);
    let startSecond=grantedSecondAbility,startThird=grantedThirdAbility,startFourth=null;
    if(challengeGrantedAbility!=null){
      if(encounter.startSecondAbilityDraft){if(startThird==null)startThird=challengeGrantedAbility;else startFourth=challengeGrantedAbility;}
      else if(startSecond==null)startSecond=challengeGrantedAbility;
      else if(startThird==null)startThird=challengeGrantedAbility;
      else startFourth=challengeGrantedAbility;
    }
    ...
    const baseHeroStartHp=Math.max(1,Number(encounter.playerHp)||START_HP);
    const masteryHpBonus=window.WDMastery?.hpBonus?.(profile,"solo",encounter)||0;
    ...
    const enemies=encounter.enemies.map((enemy,i)=>{const built=buildCampaignEnemyPlayer(enemy,encounter,i);if(enemy.ability==="mirror")built.ability=heroAbility;if(enemy.secondAbility==="mirror")built.secondAbility=heroAbility;return built;});
    players=[hero,...enemies];initializeEncounterMechanics();
    ...
    hideFrontScreens();game.classList.remove("hidden");game.classList.add("campaign-game");game.classList.remove("trio-game");document.body.classList.add("playing");window.scrollTo?.(0,0);
    ...
    if(encounter.disableBonusDraft) addLog(`🚫 Encounter-Regel: Keine zusätzlichen Fähigkeits-Drafts. ${profile.name} bleibt bei der Startfähigkeit.`);
      ...
      const masteryThreshold=window.WDMastery?.abilityThreshold?.(profile,"solo",encounter)??15;
  ...
  function finishCampaignEncounter(heroWon){
    ...
    window.WDMastery?.noteMatchEnd?.(heroIndex,heroWon);
    ...
    if(heroWon && challengeMet && profile && progress){
      ...
      // Duellmarken (js/46-shop-daten.js): Weltboss erstmals 250, Encounter
      // erstmals 80, Wiederholung 20.
      window.WDShop?.verdiene?.(profile,newlyCompleted?(CAMPAIGN_WORLDS.some(w=>w.finalEncounterId===encounter.id)?"weltboss_erst":"encounter_erst"):"encounter_wieder");
      progress.campaignVersion=Math.max(Number(progress.campaignVersion)||1,CAMPAIGN_VERSION);
    }
    const campaignComplete=profile?isCampaignComplete(profile):false;
    if(newlyCompleted && profile){
      const nextEncounter=nextSoloCampaignEncounter(profile,encounter)||defaultCampaignEncounter(profile,encounter.world||campaignWorldId);
      if(nextEncounter){
        campaignEncounterId=nextEncounter.id;
        campaignWorldId=nextEncounter.world||campaignWorldId;
        gameContext.encounterId=nextEncounter.id;
      }
    }
    ...
    if(heroWon && challengeMet){
      ...
      const markenText=window.WDShop?.gutschriftText?.(escapeHtml)||"";
      roundResultText.innerHTML=`✅ Gegner besiegt.<br>✅ Challenge erfüllt: <strong>${escapeHtml(encounter.challenge.text)}</strong>${rewardText}${secondaryRewardText}${diceRewardText}${trophyText}${markenText}${prestigeText}${masteryText}${newlyCompleted?"<br>Der nächste Encounter wurde freigeschaltet.":"<br>Dieser Encounter war bereits abgeschlossen und bleibt spielbar."}${worldUnlockText}`;
    }else if(heroWon){
    ...
    roundStandings.innerHTML=players.map((p,i)=>`<div class="round-score-row${i===heroIndex&&heroWon?" winner-row":""}"><div class="round-score-name">${escapeHtml(p.name)}</div><div class="round-score-meta">${p.campaignTeam==="hero"?"🧍 Spieler":"🤖 Gegner"} · ${Math.max(0,p.hp)} HP</div></div>`).join("");
    renderRoundStats();
