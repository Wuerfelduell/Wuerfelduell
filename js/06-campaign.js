  ...
  function campaignChallengeProgressInfo(c,heroIndex){
    ...
    switch(c.type){
      ...
      case "all_heroes_secondary_unlocked": label="2. Fähigkeit bei allen";value=heroes.map((i,n)=>`P${n+1} ${players[i]?.secondAbilityUnlocked?"✓":"–"}`).join(" · ");break;
  ...
  function renderTrioCampaign(){
    ...
    const abilitySelects=[trioAbility1Select,trioAbility2Select,trioAbility3Select],required=Array.isArray(encounter?.requiredPrimaryAbilities)?encounter.requiredPrimaryAbilities.map(Number):null,primaryOk=!required||required.length!==3||required.every((id,i)=>+abilitySelects[i].value===id);
    if(encounter){const done=!!progress?.completedEncounters?.includes(encounter.id),available=validTeam&&trioEncounterAvailable(p1,p2,p3,encounter),rules=encounterRuleText(encounter),phase=bossPhaseFor(encounter),grants=Array.isArray(encounter.grantedThirdAbilities)?encounter.grantedThirdAbilities.map((id,i)=>`P${i+1}: ${ABILITIES[id]?.name||id}`).join(" · "):"",reqText=required&&required.length===3?`P1 ${ABILITIES[required[0]]?.name} · P2 ${ABILITIES[required[1]]?.name} · P3 ${ABILITIES[required[2]]?.name}`:"",reward=done?(encounter.farmTrophy?"🏆 +1 Trophäe je Profil pro erfolgreichem Clear":"Erstclear bereits abgeschlossen"):`🏆 +1 Trophäe je Profil beim Erstclear`;trioCampaignEncounterDetail.innerHTML=`<div class="node-detail-head"><div><div class="node-detail-title">${escapeHtml(encounter.title)}</div><div class="node-detail-sub">${escapeHtml(encounter.subtitle)}</div></div><div class="node-detail-state">${done?(encounter.farmTrophy?"🏆 FARM":"✓ GESCHAFFT"):available?"OFFEN":"🔒 GESPERRT"}</div></div><div class="node-detail-desc">${escapeHtml(encounter.desc)}</div><div class="node-detail-row">🎯 <strong>Challenge:</strong> ${escapeHtml(encounter.challenge.text)}</div><div class="node-detail-row">🏆 <strong>Belohnung:</strong> ${escapeHtml(reward)}</div>${grants?`<div class="node-detail-row node-detail-phase">🎁 <strong>Extra-Fähigkeiten:</strong> ${escapeHtml(grants)}</div>`:""}${reqText?`<div class="node-detail-row node-detail-rule">🔐 <strong>Pflicht-Loadout:</strong> ${escapeHtml(reqText)}</div>`:""}${rules.map(r=>`<div class="node-detail-row node-detail-rule">⚙ <strong>${escapeHtml(r.name)}:</strong> ${escapeHtml(r.desc)}</div>`).join("")}${phase?`<div class="node-detail-row node-detail-phase">👹 <strong>Boss-Phase bei ${Math.round((phase.threshold||.5)*100)} %:</strong> ${escapeHtml(phase.title)} · ${escapeHtml(phase.desc)}</div>`:""}`;}else trioCampaignEncounterDetail.innerHTML="";
  ...
  function startTrioCampaignEncounter(){
    ...
    const selected=[+trioAbility1Select.value,+trioAbility2Select.value,+trioAbility3Select.value].map(id=>CHOOSABLE_ABILITY_IDS.includes(id)?id:(CHOOSABLE_ABILITY_IDS[0]||3)),required=Array.isArray(encounter.requiredPrimaryAbilities)?encounter.requiredPrimaryAbilities.map(Number):null;if(required&&required.length===3&&!required.every((id,i)=>selected[i]===id)){renderTrioCampaign();return;}
    ...
    logEl.innerHTML="";winnerBox.classList.add("hidden");nextRoundBox.classList.add("hidden");nextRoundPrepBtn.classList.add("hidden");restartBtn.textContent="Zur Trio-Kampagne";hideFrontScreens();game.classList.remove("hidden");game.classList.add("campaign-game","trio-game");document.body.classList.add("playing");window.scrollTo?.(0,0);addLog(`💪 Trio-Kampagne: ${encounter.title} – ${encounter.subtitle}. Challenge: ${encounter.challenge.text}`);addLog(`✨ HP-Bonus: Jeder Held erhält seine 2. Fähigkeit bei ≤${trioCampaignHpBonusThreshold(encounter)} HP.`);encounterRuleText(encounter).forEach(r=>addLog(`⚙ Sonderregel ${r.name}: ${r.desc}`));if(bossPhaseFor(encounter))addLog(`👹 Bossphase vorbereitet: ${bossPhaseFor(encounter).title} bei ${Math.round((bossPhaseFor(encounter).threshold||.5)*100)} % Boss-HP.`);const extra=heroes.map(h=>h.thirdAbility!=null?`${h.name}: ${ABILITIES[h.thirdAbility].name}`:null).filter(Boolean);if(extra.length)addLog(`🎁 Encounter-Fähigkeiten: ${extra.join(" · ")}.`);addLog(`💀 Verloren ist erst, wenn alle drei Helden gefallen sind.`);renderAll();
  }
  ...
  function startDuoCampaignEncounter(){
    ...
    const makeHero=(profile,ability)=>({name:profile.name,battleTag:`#${profile.tagNumber}`,profileId:profile.id,botLevel:"human",campaignTeam:"hero",hp:heroStartHp,maxHp:heroStartHp,ability,secondAbility:null,thirdAbility:null,secondAbilityUnlocked:sharedDuoSecondPool.length<2,thirdAbilityUnlocked:false,rolledAbility:"DUO",primaryWasChosen:true,seat:0,diceDesign:profile.selectedDice||"classic",...playerCosmeticsFromProfile(profile),wins:0,momentumStreak:0,lastStandUsed:false,roundLastStandTriggered:false,damageSinceLastOwnTurn:false,bloodRushPrimed:false,voluntaryHpPaidThisTurn:false,botBloodUsesThisAttack:0});
  ...
  function startCampaignEncounter(){
    ...
    if(grantedSecondAbility!=null && heroAbility===grantedSecondAbility){
      heroAbility=unlocked.find(id=>id!==grantedSecondAbility)??heroAbility;
    }
