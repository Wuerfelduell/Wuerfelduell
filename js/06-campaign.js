  ...
  function renderTrioCampaign(){
    ...
    trioCampaignStartBtn.disabled=!validTeam||!encounter||!trioEncounterAvailable(p1,p2,p3,encounter)||!primaryOk;trioCampaignStartBtn.title=!primaryOk?"Für diesen Encounter gilt das angezeigte Pflicht-Loadout.":"";
  }
  ...
  function startDuoCampaignEncounter(){
    ...
    addLog(`🤝 Duo-Kampagne: ${encounter.title} – ${encounter.subtitle}. Challenge: ${encounter.challenge.text}`);if(heroStartHp!==START_HP)addLog(`❤️ Encounter-Bonus: Beide Duo-Spieler starten mit ${heroStartHp} HP.`);const duoChallengeAbility=campaignChallengeGrantedAbility(encounter,[]);if(duoChallengeAbility!=null)addLog(`🎯 Challenge-Ausrüstung: ${ABILITIES[duoChallengeAbility].name} ist bei beiden Spielern automatisch dabei, sofern sie nicht bereits im Loadout steckt.`);addLog(`✨ Bonus-Draft pro Spieler: erster eigener Gegner-Kill ODER ≤15 HP – was zuerst passiert.`);encounterRuleText(encounter).forEach(r=>addLog(`⚙ Sonderregel ${r.name}: ${r.desc}`));if(bossPhaseFor(encounter))addLog(`👹 Bossphase vorbereitet: ${bossPhaseFor(encounter).title} bei ${Math.round((bossPhaseFor(encounter).threshold||.5)*100)} % Boss-HP.`);addLog(`⚡ Duo-Vollpool: Alle regulären Hauptfähigkeiten sind verfügbar. ${p1.name}: ${ABILITIES[a1].name} · ${p2.name}: ${ABILITIES[a2].name}.`);addLog(`💀 Fällt einer von euch, läuft der Encounter weiter, solange der andere noch lebt.`);renderAll();
  }
