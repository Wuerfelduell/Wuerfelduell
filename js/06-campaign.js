  ...
  function renderDuoCampaign(){
    ...
    duoCampaignStartBtn.disabled=!baseUnlocked||!worldUnlocked||!encounter||!duoEncounterAvailable(p1,p2,encounter)||!duoAbility1Select.value||!duoAbility2Select.value;window.WDMastery?.refreshAll?.();
  }
  ...
  function renderTrioCampaign(){
    ...
    if(trioCampaignWorldDesc) trioCampaignWorldDesc.textContent=world.desc;
  ...
  function startTrioCampaignEncounter(){
    ...
    };const heroes=[makeHero(p1,selected[0],0),makeHero(p2,selected[1],1),makeHero(p3,selected[2],2)];const enemies=encounter.enemies.map(enemy=>({name:enemy.name,battleTag:"",profileId:null,botLevel:enemy.level||"normal",campaignTeam:"enemy",hp:Number(enemy.hp)||START_HP,maxHp:Number(enemy.hp)||START_HP,ability:Number(enemy.ability)||0,secondAbility:enemy.secondAbility!=null?Number(enemy.secondAbility):null,thirdAbility:null,secondAbilityUnlocked:true,thirdAbilityUnlocked:true,rolledAbility:"TRIO",primaryWasChosen:false,seat:0,diceDesign:"classic",wins:0,momentumStreak:0,lastStandUsed:false,roundLastStandTriggered:false,damageSinceLastOwnTurn:false,bloodRushPrimed:false,voluntaryHpPaidThisTurn:false,botBloodUsesThisAttack:0}));players=[];const max=Math.max(heroes.length,enemies.length);for(let i=0;i<max;i++){if(heroes[i])players.push(heroes[i]);if(enemies[i])players.push(enemies[i]);}
  ...
  function startDuoCampaignEncounter(){
    ...
    const makeHero=(profile,ability)=>{const challengeAbility=campaignChallengeGrantedAbility(encounter,[ability]);const heroStartHp=baseHeroStartHp+(window.WDMastery?.hpBonus?.(profile,"duo",encounter)||0);return {name:profile.name,battleTag:`#${profile.tagNumber}`,profileId:profile.id,botLevel:"human",campaignTeam:"hero",hp:heroStartHp,maxHp:heroStartHp,campaignStartHp:heroStartHp,ability,secondAbility:challengeAbility,thirdAbility:null,fourthAbility:null,secondAbilityUnlocked:challengeAbility!=null||sharedDuoSecondPool.length<2,thirdAbilityUnlocked:false,fourthAbilityUnlocked:false,campaignBonusDraftUsed:false,rolledAbility:"DUO",primaryWasChosen:true,secondAbilityWasChosen:false,thirdAbilityWasChosen:false,fourthAbilityWasChosen:false,seat:0,diceDesign:profile.selectedDice||"classic",...playerCosmeticsFromProfile(profile),wins:0,momentumStreak:0,lastStandUsed:false,roundLastStandTriggered:false,damageSinceLastOwnTurn:false,bloodRushPrimed:false,voluntaryHpPaidThisTurn:false,botBloodUsesThisAttack:0};};
    ...
    logEl.innerHTML="";winnerBox.classList.add("hidden");nextRoundBox.classList.add("hidden");nextRoundPrepBtn.classList.add("hidden");restartBtn.textContent="Zur Duo-Kampagne";hideFrontScreens();game.classList.remove("hidden");game.classList.add("campaign-game");game.classList.remove("trio-game");document.body.classList.add("playing");window.scrollTo?.(0,0);
    addLog(`🤝 Duo-Kampagne: ${encounter.title} – ${encounter.subtitle}. Challenge: ${encounter.challenge.text}`);if(heroes.some(h=>h.hp!==START_HP))addLog(`❤️ Duo-Mastery/Encounter-HP: ${heroes.map(h=>`${h.name} ${h.hp} HP`).join(" · ")}.`);const duoChallengeAbility=campaignChallengeGrantedAbility(encounter,[]);if(duoChallengeAbility!=null)addLog(`🎯 Challenge-Ausrüstung: ${ABILITIES[duoChallengeAbility].name} ist bei beiden Spielern automatisch dabei, sofern sie nicht bereits im Loadout steckt.`);addLog(`✨ Bonus-Draft pro Spieler: erster eigener Gegner-Kill ODER ≤15 HP – was zuerst passiert.`);encounterRuleText(encounter).forEach(r=>addLog(`⚙ Sonderregel ${r.name}: ${r.desc}`));if(bossPhaseFor(encounter))addLog(`👹 Bossphase vorbereitet: ${bossPhaseFor(encounter).title} bei ${Math.round((bossPhaseFor(encounter).threshold||.5)*100)} % Boss-HP.`);addLog(`⚡ Duo-Vollpool: Alle regulären Hauptfähigkeiten sind verfügbar. ${p1.name}: ${ABILITIES[a1].name} · ${p2.name}: ${ABILITIES[a2].name}.`);addLog(`💀 Fällt einer von euch, läuft der Encounter weiter, solange der andere noch lebt.`);renderAll();
  }
