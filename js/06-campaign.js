  ...
  function trioEncounterAvailable(profile1,profile2,profile3,encounter){if(!profile1||!profile2||!profile3||new Set([profile1.id,profile2.id,profile3.id]).size!==3||!encounter)return false;const progress=trioCampaignProgress(profile1,profile2,profile3);if(progress.completedEncounters.includes(encounter.id))return true;return (encounter.requires||[]).every(id=>progress.completedEncounters.includes(id));}
  function defaultTrioEncounter(profile1,profile2,profile3){const progress=trioCampaignProgress(profile1,profile2,profile3,false);if(!progress)return TRIO_CAMPAIGN_ENCOUNTERS[0]||null;return TRIO_CAMPAIGN_ENCOUNTERS.find(e=>!progress.completedEncounters.includes(e.id)&&trioEncounterAvailable(profile1,profile2,profile3,e))||(progress.completedEncounters.includes(TRIO_CAMPAIGN_WORLD.finalEncounterId)?trioEncounterById(TRIO_CAMPAIGN_WORLD.finalEncounterId):TRIO_CAMPAIGN_ENCOUNTERS[0]);}
  ...
  function renderTrioCampaign(){
    ...
    const completed=progress?TRIO_CAMPAIGN_ENCOUNTERS.filter(e=>progress.completedEncounters.includes(e.id)).length:0,complete=!!progress?.completedEncounters?.includes(TRIO_CAMPAIGN_WORLD.finalEncounterId);trioTeamSummary.textContent=validTeam?`${p1.name} + ${p2.name} + ${p3.name}`:(profiles.length<3?"3 Profile erforderlich":"Drei verschiedene Profile wählen");trioProgressSummary.textContent=`${completed} / ${TRIO_CAMPAIGN_ENCOUNTERS.length} Encounter`;trioTrophySummary.textContent="🏆 +1 je Profil / Erstclear · L10/L15 farmbar";trioCampaignBanner.classList.toggle("hidden",validTeam);trioCampaignBanner.innerHTML=!validTeam?`🔒 <strong>Trio braucht drei verschiedene Profile.</strong>`:"";
    ...
    if(encounter){const done=!!progress?.completedEncounters?.includes(encounter.id),available=validTeam&&trioEncounterAvailable(p1,p2,p3,encounter),rules=encounterRuleText(encounter),phase=bossPhaseFor(encounter),grants=Array.isArray(encounter.grantedThirdAbilities)?encounter.grantedThirdAbilities.map((id,i)=>`P${i+1}: ${ABILITIES[id]?.name||id}`).join(" · "):"",reqText=required&&required.length===3?`P1 ${ABILITIES[required[0]]?.name} · P2 ${ABILITIES[required[1]]?.name} · P3 ${ABILITIES[required[2]]?.name}`:"",reward=done?(encounter.farmTrophy?"🏆 +1 Trophäe je Profil pro erfolgreichem Clear":"Erstclear bereits abgeschlossen"):`🏆 +1 Trophäe je Profil beim Erstclear`,challengeGear=campaignChallengeDefaultAbility(encounter);trioCampaignEncounterDetail.innerHTML=`<div class="node-detail-head"><div><div class="node-detail-title">${escapeHtml(encounter.title)}</div><div class="node-detail-sub">${escapeHtml(encounter.subtitle)}</div></div><div class="node-detail-state">${done?(encounter.farmTrophy?"🏆 FARM":"✓ GESCHAFFT"):available?"OFFEN":"🔒 GESPERRT"}</div></div><div class="node-detail-desc">${escapeHtml(encounter.desc)}</div><div class="node-detail-row">🎯 <strong>Challenge:</strong> ${escapeHtml(encounter.challenge.text)}</div><div class="node-detail-row">🏆 <strong>Belohnung:</strong> ${escapeHtml(reward)}</div>${challengeGear!=null?`<div class="node-detail-row node-detail-phase">🎯 <strong>Challenge-Ausrüstung:</strong> ${escapeHtml(ABILITIES[challengeGear]?.name||String(challengeGear))} wird allen drei Spielern zusätzlich gestellt.</div>`:""}<div class="node-detail-row node-detail-phase">✨ <strong>Bonus-Draft je Spieler:</strong> erster eigener Gegner-Kill oder ≤15 HP – was zuerst passiert.</div>${grants?`<div class="node-detail-row node-detail-phase">🎁 <strong>Extra-Fähigkeiten:</strong> ${escapeHtml(grants)}</div>`:""}${reqText?`<div class="node-detail-row node-detail-rule">🔐 <strong>Pflicht-Loadout:</strong> ${escapeHtml(reqText)}</div>`:""}${rules.map(r=>`<div class="node-detail-row node-detail-rule">⚙ <strong>${escapeHtml(r.name)}:</strong> ${escapeHtml(r.desc)}</div>`).join("")}${phase?`<div class="node-detail-row node-detail-phase">👹 <strong>Boss-Phase bei ${Math.round((phase.threshold||.5)*100)} %:</strong> ${escapeHtml(phase.title)} · ${escapeHtml(phase.desc)}</div>`:""}`;}else trioCampaignEncounterDetail.innerHTML="";
    trioCampaignStartBtn.disabled=!validTeam||!encounter||!trioEncounterAvailable(p1,p2,p3,encounter)||!primaryOk;trioCampaignStartBtn.title=!primaryOk?"Für diesen Encounter gilt das angezeigte Pflicht-Loadout.":"";window.WDMastery?.refreshAll?.();
  }
