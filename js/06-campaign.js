  ...
  function renderTrioCampaign(){
    ...
    if(encounter){const done=!!progress?.completedEncounters?.includes(encounter.id),available=validTeam&&trioEncounterAvailable(p1,p2,p3,encounter),rules=encounterRuleText(encounter),grants=Array.isArray(encounter.grantedThirdAbilities)?encounter.grantedThirdAbilities.map((id,i)=>`P${i+1}: ${ABILITIES[id]?.name||id}`).join(" · "):"",reqText=required&&required.length===3?`P1 ${ABILITIES[required[0]]?.name} · P2 ${ABILITIES[required[1]]?.name} · P3 ${ABILITIES[required[2]]?.name}`:"",reward=done?(encounter.farmTrophy?"🏆 +1 Trophäe je Profil pro erfolgreichem Clear":"Erstclear bereits abgeschlossen"):`🏆 +1 Trophäe je Profil beim Erstclear`;trioCampaignEncounterDetail.innerHTML=`<div class="node-detail-head"><div><div class="node-detail-title">${escapeHtml(encounter.title)}</div><div class="node-detail-sub">${escapeHtml(encounter.subtitle)}</div></div><div class="node-detail-state">${done?(encounter.farmTrophy?"🏆 FARM":"✓ GESCHAFFT"):available?"OFFEN":"🔒 GESPERRT"}</div></div><div class="node-detail-desc">${escapeHtml(encounter.desc)}</div><div class="node-detail-row">🎯 <strong>Challenge:</strong> ${escapeHtml(encounter.challenge.text)}</div><div class="node-detail-row">🏆 <strong>Belohnung:</strong> ${escapeHtml(reward)}</div>${grants?`<div class="node-detail-row node-detail-phase">🎁 <strong>Extra-Fähigkeiten:</strong> ${escapeHtml(grants)}</div>`:""}${reqText?`<div class="node-detail-row node-detail-rule">🔐 <strong>Pflicht-Loadout:</strong> ${escapeHtml(reqText)}</div>`:""}${rules.map(r=>`<div class="node-detail-row node-detail-rule">⚙ <strong>${escapeHtml(r.name)}:</strong> ${escapeHtml(r.desc)}</div>`).join("")}`;}else trioCampaignEncounterDetail.innerHTML="";
    trioCampaignStartBtn.disabled=!validTeam||!encounter||!trioEncounterAvailable(p1,p2,p3,encounter)||!primaryOk;trioCampaignStartBtn.title=!primaryOk?"Für dieses Finale gilt das angezeigte Pflicht-Loadout.":"";
  }
