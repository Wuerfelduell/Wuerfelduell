(() => {
  ...
  const REWARDS=Object.freeze([
    ...
    {kind:"perk",id:"second_find",rarity:"epic",name:"Zweitfund",icon:"duo.svg",desc:"Die nächsten drei Stufen erhält der Partner je eine deiner Belohnungen eine Stufe später ebenfalls. Ein erneuter Fund verlängert um drei Stufen."},
  ...
  function masteryTargets(profileId,rewardId){
    ...
    return ABILITY_SLOTS.flatMap((slot,index)=>{
      ...
      const effective=Math.max(window.WDMastery?.abilityLevel?.(getProfile(profileId),"duo",id)||0,abilityLevelOverride(profileId,id));
  ...
  function copyActions(task){
    ...
    if(ziel!=null)knoepfe.push(optionButton(tr("Weitergeben"),`${tr("Diese Belohnung geht stattdessen an")} ${getProfile(ziel)?.name||""}.`,"duo.svg","data-rush-pass","1",tr("Zweitfund")));
  ...
  function renderRewardTurn(){
    ...
    if(run.swapPending&&MASTERY_REWARD_LEVELS[run.swapPending.rewardId]){
      ...
      $("duoBossRushRewardOptions").innerHTML=masteryTargets(task.profileId,reward.id).map(target=>
    ...
    $("duoBossRushRewardOptions").innerHTML=rewardChoices.map(choice=>optionButton(tr(choice.name),tr(choice.desc),choice.icon,"data-boss-rush-reward",choice.id,choiceStateLabel(task.profileId,choice))).join("")+copyActions(task);
  ...
  function showOutcome(completed,technicalMessage=""){
    ...
    const heroRows=run.profileIds.map((profileId,slot)=>{
      ...
      return `<div class="round-score-row${completed?" winner-row":""}"><div class="round-score-name">${safe(profile?.name||tr(`Spieler ${slot+1}`))}</div><div class="round-score-meta">${safe(tr(`Duo-Spieler · ${Math.max(0,hero?.hp||0)} HP · Boss XP gesamt ${profileBossXp(profile)}`))} · ${safe(perkSummary(profileId))}</div></div>`;
    }).join("");
    ...
    restartBtn.textContent=tr("Zur Duo-Kampagne");
  ...
  function refreshButton(){
    ...
    button.title=tr(!validPair?"Zwei verschiedene Duo-Profile wählen":!unlocked?"Duo-Kampagne zuerst freischalten":"10 Bossstufen · wechselnde Loadouts · Build-Drafts nach jeder Stufe");
    if(summary){
      summary.textContent=validPair
        ?`Boss XP · ${p1.name} ${profileBossXp(p1)} · ${p2.name} ${profileBossXp(p2)}`
        :tr("Boss XP · zwei Profile wählen");
    }
  }
