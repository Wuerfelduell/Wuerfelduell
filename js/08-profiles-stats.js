  ...
  function renderPrestigeShop(){
    ...
    const title=titleItem?.name||"Kein Titel";
    const frame=frameItem?.name||"Kein Rahmen";
    ...
    prestigeEquipped.innerHTML=`<strong>Aktiv</strong><div class="prestige-active-list"><span class="prestige-active-chip">${uiIcon("navigation/info.svg")}<b>Titel</b> ${escapeHtml(title)}</span><span class="prestige-active-chip">${uiIcon("gameplay/prestige.svg")}<b>Rahmen</b> ${escapeHtml(frame)}</span><span class="prestige-active-chip">${uiIcon("gameplay/dice.svg")}<b>Würfel</b> ${escapeHtml(dice)}</span><span class="prestige-active-chip">${uiIcon("gameplay/attack.svg")}<b>Effekt</b> ${escapeHtml(fx)}</span></div><div class="prestige-reset-row"><button type="button" class="secondary" data-reset-cosmetic="title">Titel entfernen</button><button type="button" class="secondary" data-reset-cosmetic="frame">Rahmen entfernen</button><button type="button" class="secondary" data-reset-cosmetic="attackfx">Effekt auf Arc Shot</button></div>`;
    prestigeShopList.innerHTML=PRESTIGE_SHOP_ITEMS.map(item=>{const owned=prestigeItemOwned(profile,item),equipped=prestigeItemEquipped(profile,item),afford=trophies>=item.cost;const typeName=item.type==="dice"?"Würfel":item.type==="frame"?"Rahmen":item.type==="attackfx"?"Angriffseffekt":"Titel";const button=equipped?`<button disabled>${uiIcon("gameplay/completed.svg")} Aktiv</button>`:owned?`<button class="good" data-shop-equip="${item.id}">Ausrüsten</button>`:`<button class="gold" data-shop-buy="${item.id}" ${afford?"":"disabled"}>${uiIcon("gameplay/trophy.svg")} ${item.cost} · Kaufen</button>`;return `<div class="prestige-item${owned?" owned":""}${item.cost>=25?" expensive":""}"><div class="prestige-item-kicker">${typeName} · ${uiIcon("gameplay/trophy.svg")} ${item.cost}</div><div class="prestige-item-name">${escapeHtml(item.name)}</div><div class="prestige-item-desc">${escapeHtml(item.desc)}</div>${button}</div>`;}).join("");
  ...
  function renderAchievements(){
    ...
    achievementList.innerHTML=Object.entries(ACHIEVEMENTS).map(([id,a])=>{
      ...
      const owners=profiles.length?profiles.map(p=>`<span class="achievement-owner${p.achievements[id]?" done":""}">${p.achievements[id]?uiIcon("gameplay/completed.svg"):uiIcon("gameplay/locked.svg")} ${escapeHtml(p.name)} <span class="battle-tag">#${escapeHtml(p.tagNumber)}</span></span>`).join(""):`<span class="achievement-owner">Noch keine Profile</span>`;
